...
import { serialize, deserialize } from 'json-serializer';
...
export class PostController {
    ...
    private async getAllPostsByTargetUser(
        ...
        try {
            ...
            response.json(posts); // Send the serialized data
        } catch (error) {
            next(error);
        }
    }
    ...
    private async updatePost(
        ...
        try {
            ...
            if (request?.files?.images) {
                const imagesArr = await imagesHandler.extractImagesArrFromRequest(request)
                imagesHandler.configureFileSaver("1-assets", "posts-images");
